...
def feed_languagesindex():
    ...
    if auth.current_user().filter_language() == "all":
        ...
        languages = (
            ...
            .filter(db.Languages.lang_code == auth.current_user().filter_language())
            .all()
        )
        languages[0].name = isoLanguages.get_language_name(
            get_locale(), languages[0].lang_code
        )
    pagination = Pagination(
        (int(off) / (int(config.config_books_per_page)) + 1),
        config.config_books_per_page,
...
def feed_shelfindex():
    ...
    return render_xml_template(
        "feed.xml", listelements=shelf, folder="opds.feed_shelf", pagination=pagination
    )
...
def feed_search(term):
    if term:
        ...
        return render_xml_template(
            "feed.xml", searchterm=term, entries=entries, pagination=pagination
        )
    else:
